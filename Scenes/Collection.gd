...
const PACK_NOTICE_OFFSET_LEFT: float = -216.0
...
const COLLECTION_PAGER_HEIGHT: float = 72.0
...
func _bind_scene_ui() -> void:
	...
	pack_panel = get_node_or_null("PackNoticePanel") as Control
	if pack_panel == null:
		create_pack_inventory_ui()
	if pack_panel != null:
		pack_inventory = pack_panel.get_node_or_null("PackNoticeRoot/PackInventory") as Control
		pack_inventory_label = null
...
func _configure_pack_notice_panel() -> void:
	if pack_panel == null:
		return
	pack_panel.clip_contents = false
	pack_panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
	pack_panel.z_as_relative = false
	pack_panel.z_index = 25

	var pack_root := pack_panel.get_node_or_null("PackNoticeRoot") as Control
	if pack_root == null:
		return
	pack_root.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	pack_root.size_flags_vertical = Control.SIZE_EXPAND_FILL
	pack_root.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _build_ui() -> void:
	...
	pager.custom_minimum_size = Vector2(0, COLLECTION_PAGER_HEIGHT)
	...
	previous_button.size_flags_vertical = Control.SIZE_SHRINK_END
	...
	page_label.size_flags_vertical = Control.SIZE_SHRINK_END
	...
	next_button.size_flags_vertical = Control.SIZE_SHRINK_END
	next_button.text = ">"
...
func _setup_magnifier() -> void:
	if magnifier_backbuffer == null or !is_instance_valid(magnifier_backbuffer):
		...
		magnifier_backbuffer.z_index = MAGNIFIER_LENS_Z_INDEX - 1
	...
	if magnifier_group == null or !is_instance_valid(magnifier_group):
		...
		magnifier_group.z_index = MAGNIFIER_LENS_Z_INDEX
		magnifier_group.visible = false
		add_child(magnifier_group)
		_build_magnifier_lens_visuals()

	_bind_magnifier_case_button()
	_sync_magnifier_z_order()

	_layout_magnifier_case_button()
	_set_magnifier_enabled(magnifier_enabled)

func _sync_magnifier_z_order() -> void:
	if magnifier_backbuffer != null and is_instance_valid(magnifier_backbuffer):
		magnifier_backbuffer.z_as_relative = false
		magnifier_backbuffer.z_index = MAGNIFIER_LENS_Z_INDEX - 1
	if magnifier_case_button != null and is_instance_valid(magnifier_case_button):
		magnifier_case_button.z_as_relative = false
		magnifier_case_button.z_index = MAGNIFIER_CASE_Z_INDEX
	if magnifier_group != null and is_instance_valid(magnifier_group):
		magnifier_group.z_as_relative = false
		magnifier_group.z_index = MAGNIFIER_LENS_Z_INDEX
...
func create_pack_inventory_ui() -> void:
	...
	pack_panel.name = "PackNoticePanel"
	add_child(pack_panel)
	pack_panel.anchor_left = 1.0
	pack_panel.anchor_right = 1.0
	pack_panel.offset_left = PACK_NOTICE_OFFSET_LEFT
	pack_panel.offset_top = PACK_NOTICE_OFFSET_TOP
	pack_panel.offset_right = PACK_NOTICE_OFFSET_RIGHT
	pack_panel.offset_bottom = PACK_NOTICE_OFFSET_BOTTOM
	pack_panel.grow_horizontal = Control.GROW_DIRECTION_BEGIN
	pack_panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
	pack_panel.clip_contents = false

	var pack_root := Control.new()
	pack_root.name = "PackNoticeRoot"
	pack_panel.add_child(pack_root)
	pack_root.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	pack_root.size_flags_vertical = Control.SIZE_EXPAND_FILL
	pack_root.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	pack_root.mouse_filter = Control.MOUSE_FILTER_IGNORE
	pack_inventory_label = null

	pack_inventory = Control.new()
	pack_inventory.name = "PackInventory"
	pack_root.add_child(pack_inventory)
	pack_inventory.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	pack_inventory.size_flags_vertical = Control.SIZE_EXPAND_FILL
	pack_inventory.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	pack_inventory.mouse_filter = Control.MOUSE_FILTER_IGNORE
	pack_inventory.clip_contents = false
...
func _create_print_count_badge(owned_count: int) -> Control:
	...
	badge.custom_minimum_size = DECK_SLOT_REMOVE_BUTTON_SIZE
	badge.size = DECK_SLOT_REMOVE_BUTTON_SIZE
	...
	number.draw.connect(func() -> void:
		_draw_print_count_badge_number(number, str(owned_count), 11 if owned_count < 10 else 9)
	)
	return badge

func _draw_print_count_badge_number(canvas: Control, text: String, font_size: int) -> void:
	if canvas == null or CODEX_TITLE_FONT == null:
		return

	var canvas_size: Vector2 = canvas.size
	if canvas_size.x <= 0.0 or canvas_size.y <= 0.0:
		canvas_size = DECK_SLOT_REMOVE_BUTTON_SIZE

	var text_size: Vector2 = CODEX_TITLE_FONT.get_string_size(text, HORIZONTAL_ALIGNMENT_LEFT, -1.0, font_size)
	var ascent: float = CODEX_TITLE_FONT.get_ascent(font_size)
	var descent: float = CODEX_TITLE_FONT.get_descent(font_size)
	var text_height: float = ascent + descent
	var draw_position := Vector2(
		(canvas_size.x - text_size.x) * 0.5,
		(canvas_size.y - text_height) * 0.5 + ascent
	)
...
func _get_card_visual_screen_rect(card_visual: CardVisual) -> Rect2:
	...
	var max_position := Vector2(
		maxf(maxf(corner_a.x, corner_b.x), maxf(corner_c.x, corner_d.x)),
		maxf(maxf(corner_a.y, corner_b.y), maxf(corner_c.y, corner_d.y))
	)
