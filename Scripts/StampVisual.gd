...
const BURN_SHADER = preload("res://Shaders/stamp_burn.gdshader")
const GRAYSCALE_SHADER = preload("res://Shaders/stamp_grayscale.gdshader")
const STAMP_ART_MASK_SHADER = preload("res://Shaders/stamp_art_mask.gdshader")
const STAMP_FRONT_TEXTURE = preload("res://Assets/stamp_base.svg")
const STAMP_BACK_TEXTURE = preload("res://Assets/stamp_back.svg")
...
const STAMP_TEXTURE_FILTER: TextureFilter = CanvasItem.TEXTURE_FILTER_LINEAR
const STAMP_ART_TEXTURE_FILTER: TextureFilter = CanvasItem.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
const STAMP_SHIMMER_ENABLED: bool = false
...
signal drag_started(stamp_visual: StampVisual)
signal drag_moved(stamp_visual: StampVisual)
signal drag_released(stamp_visual: StampVisual)
signal burn_finished(stamp_visual: StampVisual)
...
@onready var stamp_face: TextureRect = $StampFace
@onready var type_frame: TextureRect = $TypeFrame
@onready var stamp_art: TextureRect = $StampArt
...
@onready var pattern_view: StampPatternView = $PatternView

var stamp: Stamp
var stamp_print: StampPrint
var face_material: ShaderMaterial
var stamp_art_material: ShaderMaterial
...
func _ready() -> void:
	...
	face_material = stamp_face.material.duplicate() as ShaderMaterial
	stamp_face.material = face_material
	stamp_art_material = ShaderMaterial.new()
	stamp_art_material.shader = STAMP_ART_MASK_SHADER
	stamp_art.material = stamp_art_material
	resized.connect(_on_stamp_visual_resized)
	shimmer_material = shimmer.material.duplicate() as ShaderMaterial
	shimmer.material = shimmer_material
	shimmer.visible = _is_stamp_shimmer_enabled() && !face_down
...
func _apply_texture_filter() -> void:
	texture_filter = STAMP_TEXTURE_FILTER
	shadow.texture_filter = STAMP_TEXTURE_FILTER
	stamp_face.texture_filter = STAMP_TEXTURE_FILTER
	type_frame.texture_filter = STAMP_ART_TEXTURE_FILTER
	stamp_art.texture_filter = STAMP_ART_TEXTURE_FILTER
	stamp_art.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	stamp_art.stretch_mode = TextureRect.STRETCH_SCALE
	effect_icon_texture.texture_filter = STAMP_TEXTURE_FILTER

func _on_stamp_visual_resized() -> void:
	pivot_offset = size * 0.5
	_update_stamp_art_sampling_uniforms(_get_stamp_art_texture())

func _update_stamp_art_sampling_uniforms(art_texture: Texture2D) -> void:
	if stamp_art_material == null:
		return
	...
	var target_size: Vector2 = stamp_art.size
	if target_size.x <= 0.0 or target_size.y <= 0.0:
		target_size = size
	if target_size.x <= 0.0 or target_size.y <= 0.0:
		target_size = Vector2.ONE

	stamp_art_material.set_shader_parameter("art_texture_size", texture_size)
	stamp_art_material.set_shader_parameter("art_target_size", target_size)
...
func set_stamp(value: Stamp) -> void:
	stamp = value
	stamp_print = null
	if is_node_ready():
		_apply_stamp()

func set_stamp_print(value: StampPrint) -> void:
	stamp_print = value
	stamp = StampPrintLibrary.get_stamp_for_print(value)
	if is_node_ready():
		_apply_stamp()
...
func play_burn_away_and_free() -> void:
	...
	if burn_snapshot == null:
		stamp_face.material = burn_material
...
func create_burn_snapshot_rect(burn_material: ShaderMaterial) -> TextureRect:
	var snapshot_texture: Texture2D = await create_stamp_snapshot_texture()
	if snapshot_texture == null:
		return null

	set_stamp_content_visible(false)
...
func create_stamp_snapshot_texture() -> Texture2D:
	...
	var snapshot_source: StampVisual = duplicate() as StampVisual
	...
	prepare_stamp_snapshot_source(snapshot_source)
...
func prepare_stamp_snapshot_source(snapshot_source: StampVisual) -> void:
	...
	if stamp_print != null:
		snapshot_source.set_stamp_print(stamp_print)
	else:
		snapshot_source.set_stamp(stamp)
	snapshot_source.set_face_down(face_down)
	snapshot_source.set_stamp_content_visible(true)
...
func set_stamp_content_visible(value: bool) -> void:
	stamp_face.visible = value
	type_frame.visible = value && !face_down && stamp != null && type_frame.texture != null
	stamp_art.visible = value && !face_down && stamp_art.texture != null
	duration_label.visible = value && !face_down
	effect_icon_texture.visible = value && !face_down && stamp != null && stamp.has_effect() && stamp.effect_icon != null
	effect_icon_label.visible = value && !face_down && stamp != null && stamp.has_effect() && stamp.effect_icon == null
...
func _apply_stamp() -> void:
	if stamp == null:
		...
		seeker_icon_label.visible = false
		pattern_view.set_stamp(null)
	else:
		name_label.text = stamp.stamp_name
		duration_label.text = "INF" if stamp.duration < 0 else str(stamp.duration)
		effect_icon_texture.texture = stamp.effect_icon
		effect_icon_label.text = StampEffect.get_effect_label(stamp.effect_type)
		pattern_view.set_stamp(stamp)
...
func _apply_art_state() -> void:
	...
	var art_texture: Texture2D = _get_stamp_art_texture()
	var stamp_mask_texture: Texture2D = _get_stamp_art_mask_texture()
	var has_stamp_art: bool = art_texture != null
	var has_stamp_mask: bool = stamp_mask_texture != null
	var uses_masked_art: bool = _uses_masked_stamp_art()

	type_frame.texture = type_frame_texture
	type_frame.visible = !face_down && stamp != null && type_frame_texture != null

	stamp_art.texture = art_texture if has_stamp_art else null
	stamp_art.visible = !face_down && has_stamp_art
	_update_stamp_art_sampling_uniforms(art_texture)
	if uses_masked_art && stamp_art_material != null:
		stamp_art.stretch_mode = TextureRect.STRETCH_SCALE
		stamp_art.material = stamp_art_material
		stamp_art_material.set_shader_parameter("type_mask_texture", type_mask_texture)
		stamp_art_material.set_shader_parameter("stamp_mask_texture", stamp_mask_texture if has_stamp_mask else type_mask_texture)
		stamp_art_material.set_shader_parameter("has_stamp_mask", has_stamp_mask)
	else:
		stamp_art.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
		stamp_art.material = null

func _get_stamp_art_texture() -> Texture2D:
	if stamp_print != null && stamp_print.stamp_art != null:
		return stamp_print.stamp_art
	if stamp != null:
		return stamp.stamp_art
	return null

func _get_stamp_art_mask_texture() -> Texture2D:
	if stamp_print != null && stamp_print.stamp_art_mask != null:
		return stamp_print.stamp_art_mask
	if stamp_print != null && stamp_print.variant_id == "full_art":
		return STAMP_FRONT_TEXTURE
	if stamp != null:
		return stamp.stamp_art_mask
	return null

func _uses_masked_stamp_art() -> bool:
	if stamp_print != null:
		return stamp_print.uses_masked_art()
	return true

func _is_stamp_shimmer_enabled() -> bool:
	if STAMP_SHIMMER_ENABLED:
		return true
	return stamp_print != null && stamp_print.stamp_shimmer_enabled

func _get_type_frame_texture() -> Texture2D:
	if stamp != null && MoveRules.is_shared_stamp(stamp):
		return SHARED_TYPE_FRAME_TEXTURE
	if stamp != null && MoveRules.is_seeker_stamp(stamp):
		return SEEKER_TYPE_FRAME_TEXTURE
	if stamp != null && MoveRules.is_se_tenant_stamp(stamp):
		return SE_TENANT_TYPE_FRAME_TEXTURE
	if stamp != null && MoveRules.is_overseal_stamp(stamp):
...
func _apply_face_state() -> void:
	var has_effect_icon: bool = stamp != null && stamp.has_effect()
	name_label.visible = !face_down
	duration_label.visible = !face_down
	effect_icon_texture.visible = !face_down && has_effect_icon && stamp.effect_icon != null
	effect_icon_label.visible = !face_down && has_effect_icon && stamp.effect_icon == null
	seeker_icon_label.visible = false
	pattern_view.visible = !face_down
	shimmer.visible = _is_stamp_shimmer_enabled() && !face_down
	_apply_art_state()
	stamp_face.texture = STAMP_BACK_TEXTURE if face_down else STAMP_FRONT_TEXTURE
	stamp_face.material = null if face_down else face_material
	if face_down:
		rotation = 0.0
		scale = rest_scale
...
func _apply_collection_state() -> void:
	...
	stamp_face.texture = STAMP_FRONT_TEXTURE
	stamp_face.material = grayscale_material
	self_modulate = Color.WHITE
	shimmer.visible = false
	draggable = false
	disabled = false
