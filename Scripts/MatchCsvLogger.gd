...
const MOVE_HEADERS: Array = [
	...
	"captured_card_code", "captured_card_duration", "captured_turns_remaining", "captured_nexus",
	"was_moving_nexus", "distance_to_enemy_base_before", "distance_to_enemy_base_after",
...
func create_move_context(game_state: GameStateData, player_id: int, from_pos: Vector2, to_pos: Vector2, piece: Piece, captured_piece: Piece) -> Dictionary:
	...
	return {
		...
		"captured_nexus": CardEffectResolver.is_nexus_piece(captured_piece),
		"was_moving_nexus": CardEffectResolver.is_nexus_piece(piece),
...
func log_move_event(game_state: GameStateData, context: Dictionary, win_condition_after: String = "") -> void:
	...
	var row: Dictionary = {
		...
		"captured_nexus": bool(context.get("captured_nexus", false)),
		"was_moving_nexus": bool(context.get("was_moving_nexus", false)),
