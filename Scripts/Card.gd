...
@export_enum("none", "shared_control", "steal_card", "move_base", "invisible_to_enemy", "invalid_squares", "frozen_squares", "bomb") var effect_type: String = CardEffect.TYPE_NONE
	...
	for x in range(rows):
		for y in range(cols):
			if x == center_x and y == center_y:
				continue
			if int(movement_pattern[x][y]) != CardEffect.MOVEMENT_NONE:
