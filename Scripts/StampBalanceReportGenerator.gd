extends RefCounted
class_name StampBalanceReportGenerator

const REPORT_FILE_NAME: String = "stamp_balance_report.csv"
const MATCHES_FILE_NAME: String = "matches.csv"
const STAMP_EVENTS_FILE_NAME: String = "stamp_events.csv"
...
func refresh_report(log_dir: String) -> bool:
	...
	var stamp_events: Array[Dictionary] = read_csv_rows(cleaned_log_dir, STAMP_EVENTS_FILE_NAME)
	...
	var report_rows: Array[Dictionary] = build_report_rows(matches, stamp_events, move_events, effect_events)
...
func refresh_session_balance(log_dir: String, session_id: String) -> bool:
	...
	var stamp_events: Array[Dictionary] = filter_rows_by_session(read_csv_rows(cleaned_log_dir, STAMP_EVENTS_FILE_NAME), cleaned_session_id)
	...
	var raw_rows: Array[Dictionary] = build_raw_report_rows(matches, stamp_events, move_events, effect_events)
	if raw_rows.is_empty():
		return false

	return StampBalanceStore.save_session_balance(cleaned_log_dir, cleaned_session_id, raw_rows, matches.size())

func build_report_rows(
	matches: Array[Dictionary],
	stamp_events: Array[Dictionary],
	move_events: Array[Dictionary],
	effect_events: Array[Dictionary]
) -> Array[Dictionary]:
	return format_report_rows(build_raw_report_rows(matches, stamp_events, move_events, effect_events))
...
func build_raw_report_rows(
	...
	var stamp_names: Dictionary = {}
	var seen_by_stamp: Dictionary = {}
	var attach_by_stamp: Dictionary = {}
	var expire_by_stamp: Dictionary = {}
	var return_deck_by_stamp: Dictionary = {}
	var return_hand_by_stamp: Dictionary = {}
	var attach_by_slot_stamp: Dictionary = {}
	var seen_by_slot_stamp: Dictionary = {}

	for event_row: Dictionary in stamp_events:
		var stamp_name: String = str(event_row.get("stamp_name", ""))
		if stamp_name.is_empty():
			continue
		stamp_names[stamp_name] = true
		var event_type: String = str(event_row.get("event_type", ""))
		var slot_key: String = make_slot_key(str(event_row.get("match_id", "")), parse_int(event_row.get("player_id", -1), -1), stamp_name)
		match event_type:
			"starting_hand", "draw_stamp":
				increment_count(seen_by_stamp, stamp_name)
				increment_count(seen_by_slot_stamp, slot_key)
			"attach_stamp":
				increment_count(attach_by_stamp, stamp_name)
				increment_count(attach_by_slot_stamp, slot_key)
			"expire_stamp":
				increment_count(expire_by_stamp, stamp_name)
			"return_to_deck":
				increment_count(return_deck_by_stamp, stamp_name)
			"return_to_hand":
				increment_count(return_hand_by_stamp, stamp_name)

	var moves_by_stamp: Dictionary = {}
	var captures_by_stamp: Dictionary = {}
	var seeker_captures_by_stamp: Dictionary = {}
	var base_entries_by_stamp: Dictionary = {}
	var progress_by_stamp: Dictionary = {}
	var move_wins_by_stamp: Dictionary = {}
	var move_by_slot_stamp: Dictionary = {}
	var progress_by_slot_stamp: Dictionary = {}
	var capture_by_slot_stamp: Dictionary = {}

	for move_row: Dictionary in move_events:
		var move_stamp_name: String = str(move_row.get("stamp_name", ""))
		if move_stamp_name.is_empty():
			continue
		stamp_names[move_stamp_name] = true
		var move_match_id: String = str(move_row.get("match_id", ""))
		var move_player_id: int = parse_int(move_row.get("player_id", -1), -1)
		var move_slot_key: String = make_slot_key(move_match_id, move_player_id, move_stamp_name)
		var base_progress: float = parse_float(move_row.get("base_progress", 0.0), 0.0)

		increment_count(moves_by_stamp, move_stamp_name)
		increment_count(move_by_slot_stamp, move_slot_key)
		increment_float(progress_by_stamp, move_stamp_name, base_progress)
		increment_float(progress_by_slot_stamp, move_slot_key, base_progress)

		if int(winner_by_match.get(move_match_id, -1)) == move_player_id:
			increment_count(move_wins_by_stamp, move_stamp_name)
		if str(move_row.get("was_capture", "")) == "true":
			increment_count(captures_by_stamp, move_stamp_name)
			increment_count(capture_by_slot_stamp, move_slot_key)
		if str(move_row.get("captured_seeker", "")) == "true":
			increment_count(seeker_captures_by_stamp, move_stamp_name)
		if str(move_row.get("did_enter_enemy_base", "")) == "true":
			increment_count(base_entries_by_stamp, move_stamp_name)

	var effects_by_stamp: Dictionary = {}
	var base_change_effects_by_stamp: Dictionary = {}
	var stamps_moved_effects_by_stamp: Dictionary = {}
	var affected_effects_by_stamp: Dictionary = {}

	for effect_row: Dictionary in effect_events:
		var effect_stamp_name: String = str(effect_row.get("stamp_name", ""))
		if effect_stamp_name.is_empty():
			continue
		stamp_names[effect_stamp_name] = true
		increment_count(effects_by_stamp, effect_stamp_name)
		...
		if !base_before.is_empty() and !base_after.is_empty() and base_before != base_after:
			increment_count(base_change_effects_by_stamp, effect_stamp_name)
		increment_count(stamps_moved_effects_by_stamp, effect_stamp_name, parse_int(effect_row.get("stamps_moved", 0), 0))
		increment_count(affected_effects_by_stamp, effect_stamp_name, parse_int(effect_row.get("affected_count", 0), 0))

	var raw_rows: Array[Dictionary] = []
	var stamp_name_list: Array = stamp_names.keys()
	stamp_name_list.sort()
	for stamp_name_value in stamp_name_list:
		var stamp_name_string: String = str(stamp_name_value)
		var association: Dictionary = calculate_stamp_association(
			stamp_name_string,
			match_ids,
			winner_by_match,
			attach_by_slot_stamp,
			move_by_slot_stamp,
			progress_by_slot_stamp,
			capture_by_slot_stamp
		)
		var move_count: int = int(moves_by_stamp.get(stamp_name_string, 0))
		var seen_count: int = int(seen_by_stamp.get(stamp_name_string, 0))
		var attach_count: int = int(attach_by_stamp.get(stamp_name_string, 0))
		var capture_count: int = int(captures_by_stamp.get(stamp_name_string, 0))
		var move_win_count: int = int(move_wins_by_stamp.get(stamp_name_string, 0))
		var progress_sum: float = float(progress_by_stamp.get(stamp_name_string, 0.0))
		raw_rows.append({
			"stamp_name": stamp_name_string,
			...
			"seeker_capture_count": int(seeker_captures_by_stamp.get(stamp_name_string, 0)),
			"base_entry_count": int(base_entries_by_stamp.get(stamp_name_string, 0)),
			...
			"expire_count": int(expire_by_stamp.get(stamp_name_string, 0)),
			"return_deck_count": int(return_deck_by_stamp.get(stamp_name_string, 0)),
			"return_hand_count": int(return_hand_by_stamp.get(stamp_name_string, 0)),
			"effect_trigger_count": int(effects_by_stamp.get(stamp_name_string, 0)),
			"base_change_effect_count": int(base_change_effects_by_stamp.get(stamp_name_string, 0)),
			"stamps_moved_by_effect": int(stamps_moved_effects_by_stamp.get(stamp_name_string, 0)),
			"affected_count_by_effect": int(affected_effects_by_stamp.get(stamp_name_string, 0)),
			...
			"seeker_stamp_lost_match_count": int(win_condition_counts.get("seeker_stamp_lost", 0)),
			"unknown_win_condition_match_count": int(win_condition_counts.get("unknown", 0)),
		})

	add_stamp_values(raw_rows)
...
func calculate_stamp_association(
	stamp_name: String,
	match_ids: Array[String],
	winner_by_match: Dictionary,
	attach_by_slot_stamp: Dictionary,
	move_by_slot_stamp: Dictionary,
	progress_by_slot_stamp: Dictionary,
	capture_by_slot_stamp: Dictionary
) -> Dictionary:
	var win_count: int = 0
	...
	for match_id: String in match_ids:
		for player_id in [0, 1]:
			var key: String = make_slot_key(match_id, player_id, stamp_name)
			if int(winner_by_match.get(match_id, -1)) == player_id:
				win_count += 1
				win_attach += float(attach_by_slot_stamp.get(key, 0))
				win_moves += float(move_by_slot_stamp.get(key, 0))
				win_progress += float(progress_by_slot_stamp.get(key, 0.0))
				win_captures += float(capture_by_slot_stamp.get(key, 0))
			else:
				lose_count += 1
				lose_attach += float(attach_by_slot_stamp.get(key, 0))
				lose_moves += float(move_by_slot_stamp.get(key, 0))
				lose_progress += float(progress_by_slot_stamp.get(key, 0.0))
				lose_captures += float(capture_by_slot_stamp.get(key, 0))
...
func add_stamp_values(rows: Array[Dictionary]) -> void:
	...
	for row: Dictionary in rows:
		...
		row["stamp_value"] = ai_score_adjustment
...
func sort_report_rows_desc(left: Dictionary, right: Dictionary) -> bool:
	return float(left.get("stamp_value", 0.0)) > float(right.get("stamp_value", 0.0))
...
func read_csv_rows(log_dir: String, file_name: String) -> Array[Dictionary]:
	...
	if file == null:
		push_warning("Could not read CSV file for stamp balance report: %s" % path)
		return []
...
func write_report(log_dir: String, rows: Array[Dictionary]) -> void:
	...
	if file == null:
		push_warning("Could not write stamp balance report: %s" % path)
		return
...
func make_slot_key(match_id: String, player_id: int, stamp_name: String) -> String:
	return "%s|%d|%s" % [match_id, player_id, stamp_name]
