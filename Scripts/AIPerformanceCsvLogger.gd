...
static func log_decision(game_state: GameStateData, player_id: int, profile: Dictionary, selected_plan: Dictionary) -> void:
	...
	var row: Dictionary = {
		...
		"strategy_mode": str(profile.get("strategy_mode", "")),
		"opening_completed": bool(profile.get("opening_completed", false)),
		"best_plan_move_score": format_float(float(profile.get("best_plan_move_score", 0.0))),
		"best_plan_setup_score": format_float(float(profile.get("best_plan_setup_score", 0.0))),
