extends RefCounted
class_name StampBalanceStore

const PROJECT_BALANCE_PATH: String = "res://Data/Balance/stamp_values.tres"
...
const INT_STAT_KEYS: Array[String] = [
	...
	"effect_stamps_moved", "effect_affected", "win_slots", "lose_slots",
]
...
static var project_balance_cache: StampBalanceData = null

static func load_project_balance() -> StampBalanceData:
	if project_balance_cache != null:
		return project_balance_cache

	var data := load(PROJECT_BALANCE_PATH) as StampBalanceData
	if data != null:
		project_balance_cache = data
		return data

	project_balance_cache = StampBalanceData.new()
	return project_balance_cache

static func get_stamp_value(stamp_name: String, fallback: float = 0.0) -> float:
	return load_project_balance().get_stamp_value(stamp_name, fallback)
...
static func promote_unpromoted_sessions(log_dir: String) -> Dictionary:
	...
	for session_path: String in session_files:
		var session_data := load(session_path) as StampBalanceData
...
static func rows_to_balance_data(raw_rows: Array[Dictionary], source_match_count: int) -> StampBalanceData:
	var data := StampBalanceData.new()
	data.schema_version = StampBalanceData.CURRENT_SCHEMA_VERSION
	...
	for row: Dictionary in raw_rows:
		var stamp_name: String = str(row.get("stamp_name", "")).strip_edges()
		if stamp_name.is_empty():
			continue
		data.stamps[stamp_name] = row_to_stats(row)
	return data

static func row_to_stats(row: Dictionary) -> Dictionary:
	return {
		"v": parse_float(row.get("stamp_value", 0.0)),
		...
		"effect_stamps_moved": parse_int(row.get("stamps_moved_by_effect", 0)),
...
static func merge_balance_data(target: StampBalanceData, session_data: StampBalanceData) -> void:
	target.source_match_count += max(0, session_data.source_match_count)
	for stamp_name_value in session_data.stamps:
		var stamp_name: String = str(stamp_name_value)
		var incoming_value = session_data.stamps.get(stamp_name, {})
		if !(incoming_value is Dictionary):
			continue
		var incoming: Dictionary = incoming_value
		var current_value = target.stamps.get(stamp_name, {})
		...
		target.stamps[stamp_name] = current
...
static func recalculate_balance_data(data: StampBalanceData) -> void:
	var rows: Array[Dictionary] = []
	for stamp_name_value in data.stamps:
		var stamp_name: String = str(stamp_name_value)
		var stats_value = data.stamps.get(stamp_name, {})
		if stats_value is Dictionary:
			var stats: Dictionary = stats_value
			rows.append(stats_to_row(stamp_name, stats, data.source_match_count, data.generated_unix, data.win_condition_counts))

	var generator := StampBalanceReportGenerator.new()
	generator.add_stamp_values(rows)
	rows.sort_custom(Callable(generator, "sort_report_rows_desc"))

	data.stamps.clear()
	for row: Dictionary in rows:
		data.stamps[str(row.get("stamp_name", ""))] = row_to_stats(row)

static func stats_to_row(stamp_name: String, stats: Dictionary, source_match_count: int, generated_unix: int, win_condition_counts: Dictionary = {}) -> Dictionary:
	...
	return {
		...
		"stamps_moved_by_effect": int(stats.get("effect_stamps_moved", 0)),
...
static func get_win_condition_counts_from_rows(raw_rows: Array[Dictionary]) -> Dictionary:
	...
	counts["seeker_stamp_lost"] = parse_int(first_row.get("seeker_stamp_lost_match_count", 0))
...
static func save_balance_data(data: StampBalanceData, path: String) -> bool:
	var error: int = ResourceSaver.save(data, path)
	if error != OK:
		push_warning("Could not save stamp balance data to %s. Error: %s" % [path, error])
		return false
	return true
