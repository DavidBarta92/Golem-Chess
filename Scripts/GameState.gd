...
func initialize_player_codex(player_id: int, card_names: Array) -> void:
	...
	sync_player_card_zones_from_codex(player_id)
...
func can_turn_page(player_id: int) -> bool:
	if bool(has_turned_page_this_turn.get(player_id, false)):
		return false
	if bool(attached_card_this_turn.get(player_id, false)):
		return false
...
func consume_stamp(player_id: int, page_index: int, stamp_index: int) -> Dictionary:
	...
	var card_name: String = str(stamps[stamp_index])
...
func consume_current_page_stamp_by_name(player_id: int, card_name: String, preferred_stamp_index: int = -1) -> Dictionary:
	var page_index: int = get_current_page_index(player_id)
	var page: Array = get_current_page(player_id)
	var stamp_index: int = -1
	if preferred_stamp_index >= 0 and preferred_stamp_index < page.size() and str(page[preferred_stamp_index]) == card_name:
		stamp_index = preferred_stamp_index
	else:
		stamp_index = page.find(card_name)
	if stamp_index == -1:
		return {}
	return consume_stamp(player_id, page_index, stamp_index)

func remove_stamp_from_codex_zone(player_id: int, card_name: String = "", include_current_page: bool = false) -> String:
	var pages: Array = get_codex_pages(player_id)
	...
	for page_index in range(pages.size()):
		...
		for stamp_index in range(stamps.size()):
			...
			if !card_name.is_empty() and candidate_name != card_name:
				continue
			candidates.append({
				"page_index": page_index,
				"stamp_index": stamp_index,
	...
	var selected_card_name: String = str(selected.get("card_name", ""))
...
func remove_stamp_from_current_page(player_id: int, card_name: String = "") -> String:
	var pages: Array = get_codex_pages(player_id)
	var page_index: int = get_current_page_index(player_id)
	...
	if stamp_index == -1:
		return ""
	var removed_card_name: String = str(page[stamp_index])
	page.remove_at(stamp_index)
	pages[page_index] = page
	player_codex_pages[player_id] = pages
	sync_player_card_zones_from_codex(player_id)
	return removed_card_name

func add_stamp_to_current_page(player_id: int, card_name: String, max_page_size: int = DeckManager.CODEX_STAMPS_PER_PAGE) -> bool:
	if card_name.is_empty():
		return false
	var pages: Array = get_codex_pages(player_id)
	var page_index: int = get_current_page_index(player_id)
	...
	if stamps.size() >= max_page_size:
		return false
	stamps.append(card_name)
	pages[page_index] = stamps
	player_codex_pages[player_id] = pages
	sync_player_card_zones_from_codex(player_id)
	return true

func return_stamp_to_codex_page(player_id: int, card_name: String, preferred_page_index: int = -1, preferred_stamp_index: int = -1) -> bool:
	if card_name.is_empty():
		return false
	...
	if target_page_index < 0 or target_page_index >= DeckManager.CODEX_PAGE_COUNT:
		var spent_record: Dictionary = pop_spent_stamp_record(player_id, card_name)
		...
		remove_matching_spent_stamp_record(player_id, card_name, target_page_index)
	...
	stamps.insert(insert_index, card_name)
	pages[target_page_index] = stamps
	player_codex_pages[player_id] = pages
	sync_player_card_zones_from_codex(player_id)
	return true

func pop_spent_stamp_record(player_id: int, card_name: String) -> Dictionary:
	var player_spent_stamps: Array = spent_stamps.get(player_id, [])
	for reverse_index in range(player_spent_stamps.size() - 1, -1, -1):
		var record = player_spent_stamps[reverse_index]
		if record is Dictionary and str(record.get("card_name", "")) == card_name:
			player_spent_stamps.remove_at(reverse_index)
			spent_stamps[player_id] = player_spent_stamps
			return record
	return {}

func remove_matching_spent_stamp_record(player_id: int, card_name: String, page_index: int) -> void:
	var player_spent_stamps: Array = spent_stamps.get(player_id, [])
	for reverse_index in range(player_spent_stamps.size() - 1, -1, -1):
		var record = player_spent_stamps[reverse_index]
		if record is Dictionary and str(record.get("card_name", "")) == card_name and int(record.get("page_index", -1)) == page_index:
			player_spent_stamps.remove_at(reverse_index)
			spent_stamps[player_id] = player_spent_stamps
			return
...
func sync_player_card_zones_from_codex(player_id: int) -> void:
	if !has_codex_for_player(player_id):
		return
	var current_page: Array = get_current_page(player_id)
	var hand: Array[String] = []
	for card_name_value in current_page:
		hand.append(str(card_name_value))
